    ...
    return (
        <>
            ...
            <nav className="fixed bottom-0 left-0 w-full md:hidden bg-surface/80 dark:bg-inverse-surface/80 backdrop-blur-xl shadow-[0px_-4px_24px_rgba(5,139,3,0.08)] z-50 rounded-t-xl">
                <div className="flex justify-around items-center px-2 py-3 pb-safe max-w-screen-xl mx-auto">
                    ...
                    {isAuthenticated && userRole === 'user' && (
                        <>
                            <button onClick={() => navigate('/user-wall')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/user-wall') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                ...
                                <span className="font-label-sm text-[10px] font-semibold">TheWall</span>
                            </button>
                            ...
                            <button onClick={() => navigate('/resources')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/resources') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                <span className="material-symbols-outlined mb-1 text-xl" style={{ fontVariationSettings: isActive('/resources') ? "'FILL' 1" : "'FILL' 0" }}>local_library</span>
                                <span className="font-label-sm text-[10px] font-semibold">Resource Library</span>
                            </button>
                        </>
                    )}
                    ...
                    {isAuthenticated && userRole === 'coach' && (
                        <>
                            ...
                            <button onClick={() => navigate('/coach-requests')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/coach-requests') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                <span className="material-symbols-outlined mb-1 text-xl" style={{ fontVariationSettings: isActive('/coach-requests') ? "'FILL' 1" : "'FILL' 0" }}>inbox</span>
                                <span className="font-label-sm text-[10px] font-semibold">Requests</span>
                            </button>
                            <button onClick={() => navigate('/resources')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/resources') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                <span className="material-symbols-outlined mb-1 text-xl" style={{ fontVariationSettings: isActive('/resources') ? "'FILL' 1" : "'FILL' 0" }}>local_library</span>
                                <span className="font-label-sm text-[10px] font-semibold">Resource Library</span>
                            </button>
                            <button onClick={(handleLogout) => navigate('/login')} className="flex flex-col items-center justify-center text-on-surface-variant hover:text-error px-2 py-1 transition-colors cursor-pointer">
                                <span className="material-symbols-outlined mb-1 text-xl">logout</span>
                                <span className="font-label-sm text-[10px] font-semibold">Logout</span>
                            </button>
                        </>
                    )}
                    ...
                    {isAuthenticated && userRole === 'admin' && (
                        <>
                            <button onClick={() => navigate('/admin-dashboard')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/admin-dashboard') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                ...
                                <span className="font-label-sm text-[10px] font-semibold">Hub</span>
                            </button>
                            <button onClick={() => navigate('/admin-coaches')} className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${isActive('/admin-coaches') ? 'bg-primary-container text-on-primary-container rounded-full px-5 py-1.5' : 'text-on-surface-variant hover:text-primary px-2 py-1'}`}>
                                <span className="material-symbols-outlined mb-1 text-xl" style={{ fontVariationSettings: isActive('/admin-coaches') ? "'FILL' 1" : "'FILL' 0" }}>groups</span>
                                <span className="font-label-sm text-[10px] font-semibold">Coaches</span>
                            </button>
                            <button onClick={() => navigate('/login')} className="flex flex-col items-center justify-center text-on-surface-variant hover:text-error px-2 py-1 transition-colors cursor-pointer">
                                <span className="material-symbols-outlined mb-1 text-xl">logout</span>
                                <span className="font-label-sm text-[10px] font-semibold">Logout</span>
                            </button>
                        </>
