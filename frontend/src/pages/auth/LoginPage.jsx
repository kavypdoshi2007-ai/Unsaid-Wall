    ...
    const handleAuthSubmit = async (e) => {
        ...
        if (authMode === 'register') {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(data.error || 'Invalid credentials or connection fault.');
                }

                // Save secure JWT token
                if (data.token) {
        ...
        try {
            const response = await fetch(API_ENDPOINTS.USERS.REGISTER, {
                ...
                body: JSON.stringify({
                    phone_number: cleanPhone,
                    password: password,
                    display_name_pool: [generatedName],
                    role: "user"
                })
            });
            ...
            if (!response.ok) {
                throw new Error(data.error || 'Failed to complete configuration registration.');
            }
