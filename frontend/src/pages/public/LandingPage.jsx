...
export default function LandingPage() {
    ...
    const fallbackHeadshots = [
        ...
        "https://lh3.googleusercontent.com/aida-public/AB6AXuCSy_McCKYdHPcknKXNO4oAeMbh0BAdf_R0fnUgLfRADzme3-jnYzh4BhzWnPxMCrzOQqj6Jm0vXs3xcbMZY5jYlleinvw8_3b0m2oern7YvtEIMeBDb5CJIlX_2DxmhKQxEsYxSWP3FrQwMYZ2uY5dH7GJnreNOWJR7ZyodJ_4pgIkEj2zh8dE9Fv6XDHu-2lhNuYSXdyTLpRDfR169s520Kf-ZrvITQSbGibR-VXaHUv6T60ppnWPsnNXLGTGA_j3gEW3QlG5bAc",
        "https://lh3.googleusercontent.com/aida-public/AB6AXuBV0nI7o881xWKdzUZ_oJcXknmph51nJFOMnVnFw1-YeTaxMX0CzTx8_Ktxp1mN8KW9phKbNof-R8juDw47uBfN8CXVcBwp-BB20cn4f3CJhT7UT8IMeCW-J_vXUyP30jgniqD_9UxgRTqP4Oq9sa0wVMCHsxsU4-6bXtv8QCFttJIZFsSORCrwfMRDGW7EVpXNgiO2312_4qaVG5J6YyW1Axy2n0lFev8lmfOX9Y9G61nnxS7z4vKEa9rCFBVSLJ9v9HK-9fmPOSM"
    ];
    ...
    useEffect(() => {
        async function loadLiveFeed() {
            try {
                ...
                setLiveFeed(posts.slice(0, 6)); // Top 6 elements matching landing script
            } catch (error) {
                console.error('Failed to sync frontend rendering arrays:', error);
            } finally {
        ...
        async function loadTopCoaches() {
            try {
                ...
                setTopCoaches(coaches.slice(0, 3)); // Featured top 3 layout target
            } catch (error) {
                console.error('Failed to update landing page coaches context:', error);
            } finally {
    ...
    return (
        <div className="bg-background text-on-background font-body-md selection:bg-primary-fixed selection:text-on-primary-fixed scroll-smooth">
            ...
            <main className="pt-16">
                    ...
                    <div className="relative z-10 max-w-max-width mx-auto px-margin-mobile md:px-margin-desktop text-center md:text-left grid md:grid-cols-2 gap-lg items-center">
                        <div className="space-y-md">
                            ...
                            <div className="flex flex-col sm:flex-row gap-base pt-sm">
                                ...
                                <button
                                    onClick={() => {
                                        const wallSection = document.getElementById('wall-anchor');
                ...
                <section id="wall-anchor" className="py-xl bg-surface-container-lowest">
                    <div className="max-w-max-width mx-auto px-margin-mobile md:px-margin-desktop">
                        ...
                        {feedLoading ? (
                            <div className="columns-1 gap-gutter text-center py-12 text-on-surface-variant animate-pulse font-body-md">
                                Loading dynamic posts stream...
                            </div>
                        ) : (
                            <div className="columns-1 md:columns-2 lg:columns-3 gap-gutter space-y-gutter">
                                {liveFeed.map((post) => {
                                    const relativeTime = formatRelativeTime(post.created_at || post.createdAt || new Date());
                                    const rawEmotion = post.emotion || 'REFLECTION';
                                    const postTag = emotionDisplayMap[rawEmotion.toUpperCase()] || rawEmotion;
                                    ...
                                    if (Array.isArray(post.reactions)) {
                                        ...
                                        totalReactions = Object.values(post.reactions).reduce((sum, count) => sum + (Number(count) || 0), 0);
                                    }

                                    return (
                                        <div key={post.id || post._id} className="break-inside-avoid bg-white p-lg rounded-xl shadow-sm border border-outline-variant/30 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                                            <div className="flex justify-between items-start mb-sm">
                                                <span className="px-sm py-1 bg-surface-container-low text-secondary rounded-full text-caption font-label-md capitalize">{postTag}</span>
                                                <span className="text-caption text-outline">{relativeTime}</span>
                                            </div>
                                            <p className="font-body-md text-body-md text-on-surface leading-relaxed mb-md whitespace-pre-line">
                                                "{postContent}"
                                            </p>
                                            <div className="flex items-center gap-sm text-outline hover:text-error cursor-pointer transition-colors w-fit">
                                                <span className="material-symbols-outlined text-md">favorite</span>
                                                <span className="text-caption">{totalReactions} support responses</span>
                                            </div>
                                        </div>
                                    );
                                })}
                                {liveFeed.length === 0 && (
                                    <div className="break-inside-avoid text-center py-12 text-on-surface-variant font-body-lg italic">
                                        The wall is quiet right now. Be the first to leave a thought.
                                    </div>
                                )}
                            </div>
                        )}
                ...
                <section className="py-xl bg-surface-container-low">
                    <div className="max-w-max-width mx-auto px-margin-mobile md:px-margin-desktop">
                        ...
                        {coachesLoading ? (
                            <div className="text-center py-12 text-on-surface-variant animate-pulse font-body-md">
                                Loading featured expert coaches...
                            </div>
                        ) : (
                            <div className="grid md:grid-cols-3 gap-gutter">
                                {topCoaches.map((coach, index) => {
                                    const coachName = coach.name || 'Verified Expert';
                                    const coachBio = coach.bio || 'Professional consultant specializing in peer guidance solutions.';
                                    const dynamicTag = (coach.specializations && coach.specializations.length > 0)
                                        ? coach.specializations[0]
                                        : ((coach.specialties && coach.specialties.length > 0) ? coach.specialties[0] : 'Mental Wellness');
                                    const profileImage = coach.profile_image || coach.profileImage || fallbackHeadshots[index % fallbackHeadshots.length];

                                    return (
                                        <div key={coach.id || coach._id} onClick={() => navigate('/coach-directory')} className="bg-white p-lg rounded-xl text-center shadow-sm border border-transparent hover:border-primary/20 transition-all group cursor-pointer transform hover:-translate-y-1 duration-300">
                                            <div className="w-24 h-24 mx-auto mb-md rounded-full overflow-hidden border-4 border-surface-container">
                                                <img className="w-full h-full object-cover" alt={coachName} src={profileImage} />
                                            </div>
                                            <h4 className="font-headline-md text-headline-md text-on-surface">{coachName}</h4>
                                            <p className="text-primary font-label-md mb-md capitalize">{dynamicTag}</p>
                                            <p className="font-body-md text-body-md text-on-surface-variant mb-lg line-clamp-3">{coachBio}</p>
                                        </div>
                                    );
                                })}
                                {topCoaches.length === 0 && (
                                    <div className="col-span-full text-center py-12 text-on-surface-variant font-body-lg italic">
                                        No expert coaches are featured at this time.
                                    </div>
                                )}
                            </div>
                        )}
