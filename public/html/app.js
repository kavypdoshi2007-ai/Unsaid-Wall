...
function updateVisualTimer(durationInSeconds) {
    ...
    function tick() {
        if (timeRemaining <= 0) {
            ...
            if (timerElement) timerElement.innerText = "Session Ended";
            return;
        }
        ...
        if (timerElement) {
            timerElement.innerText = `Active Session: ${formattedMinutes}:${formattedSeconds} remaining`;
        }
...
function buildPostCardMarkup(post) {
    ...
    if (post && Array.isArray(post.reactions)) {
        post.reactions.forEach(item => {
            ...
            const reactionAuthorId = item.user_id || item.userId;
            if (typeKey && initialGroupsMap[typeKey] !== undefined) {
                ...
                if (currentUserToken && (reactionAuthorId === currentProfileId || item.userHasReacted === true)) {
...
async function loadCardReactionMetrics(postId) {
    ...
    try {
        ...
        if (dataArray && dataArray.length > 0) {
            const liveSumTotal = dataArray.reduce((acc, obj) => acc + (html => Number(obj.count) || 1)(obj), 0);
            ...
            dataArray.forEach(counter => {
                const typeKey = counter.reaction_type || counter.type;
                if (!typeKey) return;

                const designMetadata = reactionDisplayMap[typeKey] || { icon: "🤍", label: typeKey };
                const isMarkedByMe = counter.userHasReacted === true || (currentProfileId && (counter.user_id === currentProfileId || counter.userId === currentProfileId));

                const styleClasses = isMarkedByMe
                    ? "border-emerald-500 bg-emerald-100 text-emerald-900 font-bold ring-2 ring-emerald-500/10"
                    : "border-emerald-100 bg-emerald-50/60 text-emerald-800 font-semibold";

                const finalCount = typeof counter.count !== 'undefined' ? counter.count : 1;
                ...
                badge.innerHTML = `<span>${designMetadata.icon}</span> <span>${designMetadata.label}</span> <span class="bg-emerald-600/10 text-emerald-700 px-1.5 py-0.2 rounded font-mono text-[11px]">${finalCount}</span>`;
                
                badge.onclick = (e) => executeReactionToggle(postId, typeKey, e);
                reactionBin.appendChild(badge);

                if (isMarkedByMe && actionBar) {
                    const linkedActionBtn = actionBar.querySelector(`[data-reaction-btn="${typeKey}"]`);
                    if (linkedActionBtn) {
