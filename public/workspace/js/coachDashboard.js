...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Initial trigger when a session starts or room loads up (Default 20 min = 1200 seconds)
socket.on('session_ready_or_loaded', () => {
    updateVisualTimer(20 * 60); 
});

// 2. Synchronize when the coach accepts an extension confirmation prompt
socket.on('timer_extended', (data) => {
    alert(data.message);
    // Reset visual count down interface neatly back to 20:00 minutes
    updateVisualTimer(20 * 60);
});

// 3. Clear UI text when the server signals the end of the line
socket.on('session_ended', (data) => {
    if (countdownInterval) clearInterval(countdownInterval);
    const timerElement = document.getElementById('session-timer');
    if (timerElement) {
        timerElement.innerText = "Session Completed";
    }
});
