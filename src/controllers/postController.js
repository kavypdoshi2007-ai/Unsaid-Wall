...
const postController = {
  ...
  async createPost(req, res, next) {
    try {
      ...
      const { content, language, display_name} = req.body; // display_name, emotion, and intensity are now auto-generated
      ...
      if (classifier && !hasEmergencyWord) {
        // Runs the model -> returns format like: [{ label: 'sadness', score: 0.924 }]
        const predictions = await classifier(content);
        
        if (predictions && predictions[0]) {
          const result = predictions[0];
          const rawLabel = result.label.toLowerCase(); // 'sadness', 'anger', 'fear', 'joy', 'love', 'surprise'
          const score = result.score; // Confidence decimal value from 0.0 to 1.0

          // A. Map Hugging Face model outputs directly to your Prisma EmotionType enums
          switch (rawLabel) {
            case 'sadness':
              detectedEmotion = 'SAD';
              break;
          ...
          // Contextual sub-parsing for enums not natively exposed by basic 6-label models
          ...
          // B. Dynamically Map Model Confidence Score to IntensityLevel enums
          if (score >= 0.85) {
            detectedIntensity = 'high';
          } else if (score >= 0.60) {
          ...
          // C. Automated system flagging rules using the model output metrics
          if (score >= 0.95 && (detectedEmotion === 'SAD' || detectedEmotion === 'ANGRY' || detectedEmotion === 'OVERWHELMED')) {
          ...
          } else if (score >= 0.75 && detectedEmotion !== 'HOPEFUL') {
            is_flagged = true;
            flag_level = 'concerning';
          }
        }
